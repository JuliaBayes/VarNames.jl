...
In general this is done by storing some kind of mapping from [`VarName`s](@ref varname) to values.
...
One problem that a lack of constructiveness caused was that conditioning on values was unusually finicky.
...
Finally, historically, reading information back from a Chains object also proved to be very difficult.
For example, MCMCChains.jl breaks up all variables into its constituent scalar components.
...
(The more modern FlexiChains.jl largely solves this problem by not breaking up the variables prematurely.)
