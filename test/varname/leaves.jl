...
@testset "varname/leaves.jl" verbose = true begin
    @testset "single value: float, int" begin
        ...
        @test Set(collect(varname_and_value_leaves(@vn(x), x))) == Set([(@vn(x), x)])
        x = 2
        @test Set(varname_leaves(@vn(x), x)) == Set([@vn(x)])
        @test Set(collect(varname_and_value_leaves(@vn(x), x))) == Set([(@vn(x), x)])
    end
    ...
    @testset "Vector" begin
        ...
        x = [(; a=1), (; b=2)]
        @test Set(varname_leaves(@vn(x), x)) == Set([@vn(x[1].a), @vn(x[2].b)])
        @test Set(collect(varname_and_value_leaves(@vn(x), x))) ==
    ...
    @testset "Matrix" begin
        ...
        @test Set(varname_leaves(@vn(x), x)) ==
              Set([@vn(x[1, 1]), @vn(x[1, 2]), @vn(x[2, 1]), @vn(x[2, 2])])
    ...
    @testset "NamedTuple" begin
        x = (a=1.0, b=[2.0, 3.0])
        @test Set(varname_leaves(@vn(x), x)) == Set([@vn(x.a), @vn(x.b[1]), @vn(x.b[2])])
        @test Set(collect(varname_and_value_leaves(@vn(x), x))) ==
              Set([(@vn(x.a), x.a), (@vn(x.b[1]), x.b[1]), (@vn(x.b[2]), x.b[2])])
    end
