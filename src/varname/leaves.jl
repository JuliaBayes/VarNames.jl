...
function varname_and_value_leaves_inner(
    vn::VarName,
    val::AbstractArray{<:Union{Real,Missing}},
)
    return (
        Leaf(VarName{getsym(vn)}(Index(Tuple(I), (;), Iden()) ∘ getoptic(vn)), val[I]) for
        I in CartesianIndices(val)
    )
end
# Containers.
...
function varname_and_value_leaves_inner(vn::VarName, x::LinearAlgebra.Cholesky)
    return if x.uplo == 'L'
        varname_and_value_leaves_inner(
            VarName{getsym(vn)}(Property{:L}(Iden()) ∘ getoptic(vn)),
            x.L,
        )
    else
        varname_and_value_leaves_inner(
            VarName{getsym(vn)}(Property{:U}(Iden()) ∘ getoptic(vn)),
            x.U,
        )
    end
end
