...
"""
    _getindex_optic(collection, optic::Optic, orig_vn::VarName)
    _getindex_optic(collection, vn::VarName)

Access the value in `collection` at the location specified by the given `optic`. If a `VarName`
is provided, it is first converted to an optic using `varname_to_optic`.

Here, `collection` can be either a `VarNamedTuple` or a `PartialArray`, or a leaf value stored
within one of these.

This is semantically similar to `getvalue` but is specialised for `VarNamedTuple`
...
function _getindex_optic(vnt::VarNamedTuple, vn::VarName)
    return _getindex_optic(vnt, varname_to_optic(vn), vn)
end
function _getindex_optic(vnt::VarNamedTuple, vn::VarName, orig_vn)
    return _getindex_optic(vnt, varname_to_optic(vn), orig_vn)
end

@inline _getindex_optic(@nospecialize(x::Any), ::Iden, orig_vn) = x
@inline _getindex_optic(x::Any, o::AbstractOptic, orig_vn) = o(x)
function _getindex_optic(vnt::VarNamedTuple, optic::Property{S}, orig_vn) where {S}
    if !haskey(vnt.data, S)
        throw(KeyError(orig_vn))
    end
    return _getindex_optic(getindex(vnt.data, S), optic.child, orig_vn)
end
function _getindex_optic(pa::PartialArray, optic::Index, orig_vn)
    coptic = concretize_top_level(optic, pa.data)
    child_value =
        if _is_multiindex(pa, coptic.ix...; coptic.kw...) && coptic.child isa Index
...
function _getindex_optic(arr::AbstractArray, optic::IndexWithoutChild, orig_vn)
    coptic = concretize_top_level(optic, arr)
    return Base.getindex(arr, coptic.ix...; coptic.kw...)
end

# Fallback for everything else.
@inline _haskey_optic(x::Any, o::AbstractOptic) = canview(o, x)
function _haskey_optic(vnt::VarNamedTuple, name::VarName)
    return _haskey_optic(vnt, varname_to_optic(name))
end
@inline _haskey_optic(@nospecialize(::Any), ::Iden) = true
@inline _haskey_optic(::VarNamedTuple, ::Index) = false
function _haskey_optic(vnt::VarNamedTuple, optic::Property{S}) where {S}
    return Base.haskey(vnt.data, S) && _haskey_optic(getindex(vnt.data, S), optic.child)
end
function _haskey_optic(pa::PartialArray, ::Iden)
    # no masked indices
...
function _haskey_optic(pa::PartialArray, optic::Index)
...
function _setindex_optic!!(
    ...
    if permissions isa MustNotOverwrite && optic.child isa Iden
        if any(view(pa.mask, coptic.ix...; coptic.kw...))
            throw(MustNotOverwriteError(permissions))
        end
    end

    sub_value = if optic.child isa Iden
        # Skip recursion
        value
    else
...
function _setindex_optic!!(
    ...
    if S in names && optic.child isa Iden && permissions isa MustNotOverwrite
        throw(MustNotOverwriteError(permissions))
    end
    sub_value = if optic.child isa Iden
        # Skip recursion
...
documentation. If you are modifying this function, please read it!
"""
@inline make_leaf(@nospecialize(value::Any), ::Iden, ::Any) = value
function make_leaf(value, optic::Property{S}, template) where {S}
    sub_value = if optic.child isa Iden
        value
    else
        child_template = SharedGetProperty{S}()(template)
...
function make_leaf(value, optic::Index, template::PartialArray)
...
function make_leaf(
    ...
    coptic = concretize_top_level(optic, template)
...
function make_leaf(value, ::Index, template::Any)
    # Fallback method; this is hit for other indexable things that we don't know how to deal
    # with.
    return error(
        "VarNamedTuple currently does not support random indexed variables within any container that is not an `AbstractArray` (got type $(typeof(template))). For example, if `x` is a `Dict`, VarNamedTuple does not handle `x[\"a\"].",
    )
end

function make_sub_value(value, coptic::Index, template)
    return if coptic.child isa Iden
        value
    else
...
# by the caller.
function make_leaf_singleindex(value, coptic::Index, template)
...
function make_leaf_multiindex(value, coptic::Index, template)
    sub_value = make_sub_value(value, coptic, template)
...
hasvalue(vnt::VarNamedTuple, vn::VarName) = haskey(vnt, vn)
getvalue(vnt::VarNamedTuple, vn::VarName) = vnt[vn]
