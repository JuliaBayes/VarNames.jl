...
@generated function _merge(
    vnt1::VarNamedTuple{names1},
    vnt2::VarNamedTuple{names2},
    ::Val{true},
) where {names1,names2}
...
"""
function subset(parent_vnt::VarNamedTuple, vns)
    return mapfoldl(
        identity,
        function (acc_vnt, pair)
            name, value = pair
            return if any(vn -> subsumes(vn, name), vns)
                templated_setindex!!(acc_vnt, value, name, parent_vnt.data[getsym(name)])
            else
                acc_vnt
            end
        end,
...
function _map_pairs_recursive_pa_alb!!(pairfunc, pa::PartialArray, vn)
    ...
    et = eltype(pa)
    index_type = Index{NTuple{ndims(pa),Int},@NamedTuple{},Iden}
    new_vn_type = Core.Compiler.return_type(append_optic, Tuple{typeof(vn),index_type})
...
function _map_values_recursive_pa_alb!!(func, pa::PartialArray)
    et = eltype(pa)
    new_et =
        Core.Compiler.return_type(Tuple{typeof(_map_values_recursive!!),typeof(func),et})
    new_et = new_et == Union{} ? Any : new_et
    new_data = if new_et <: et
        pa.data
    else
...
function _map_pairs_recursive_pa_noalb!!(pairfunc, pa::PartialArray, vn)
    ...
    function inner(data_i, ix_i)
        new_vn = append_optic(vn, Index(Tuple(ix_i), (;)))
    ...
    for name in Names
        push!(
            ...
            :(_map_pairs_recursive!!(
                pairfunc,
                vnt.data.$name,
                append_optic(vn, Property{$(QuoteNode(name))}()),
            )),
        )
    end
    return quote
        ...
        push!(
            ...
            quote
                result = _mapreduce_recursive(
                    ...
                    op,
                    vnt.data.$name,
                    prefix(VarName{$(QuoteNode(name))}(), vn),
                    result,
                )
            end,
        )
