...
function concretize_top_level(idx::Index, val::GrowableArray)
    if any(ix -> ix isa DynamicIndex, idx.ix)
...
function Base.getindex(pa::PartialArray, inds::Vararg{Any}; kw...)
    ...
    elseif val isa GrowableArray
        ...
        # function.
        if any(ind -> ind isa DynamicIndex || ind isa Colon, inds)
            _warn_growable_array_extraction()
        end
...
function _remove_partial_blocks!!(
    pa_data::AbstractArray,
    pa_mask::AbstractArray{Bool},
    inds::Vararg{Any};
    kw...,
)
    et = eltype(pa_data)
...
function grow_to_indices!!(
    pa::PartialArray{T,ndims,A},
    inds::Vararg{Any,ndims};
    kw...,
...
function _subset_partialarray(pa::PartialArray, inds::Vararg{Any}; kw...)
    if pa.data isa GrowableArray && any(ind -> ind isa DynamicIndex || ind isa Colon, inds)
        _warn_growable_array_extraction()
    end
